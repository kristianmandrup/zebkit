...
export const $deviceRatio = typeof window.devicePixelRatio !== "undefined"
...
export const $taskMethod = window.requestAnimationFrame ||
                    window.webkitRequestAnimationFrame  ||
                    window.mozRequestAnimationFrame     ||
                    function(callback) { 
                        return window.setTimeout(callback, 35); 
                    };

export const $task = function(f){
...
export const $viewPortSize = function() {
    var ws   = $windowSize(),
        body = document.body,
    ...
    for(var i = 0; i < css.length;) {
        ws.width  -= ($measure(body, css[i++]) + $measure(body, css[i++]));
        ws.height -= ($measure(body, css[i++]) + $measure(body, css[i++]));
    }
    return ws;
};

export const $measure = function(e, cssprop) {
    var value = window.getComputedStyle(e, null).getPropertyValue(cssprop);
    return (value == null || value === '') ? 0
                                            : parseInt(/(^[0-9\.]+)([a-z]+)?/.exec(value)[1], 10);
};

export const $contains = function(element) {
...
export const $isInsideElement = function(element, pageX, pageY) {
...
export const $focusInOutSupported = (function() {
    var support = false,
...
export const $focusin = function(element, f, b) {
    return element.addEventListener($focusInOutSupported ? "focusin" : "focus", f, b);
};

export const $focusout = function(element, f, b) {
 ...
 * image has been completely loaded or failed. The method gets three parameters
    ...
    * @method  loadImage
    */
export const $loadImage = function(img, ready) {
    var i = null;
...
export const $eventsBlackHole = function(e) {
    e.preventDefault();
    e.stopPropagation();
};

export const $createBlockedElement = function() {
...
export const $ContextMethods = {
    ...
    drawDashLine : function(x,y,x2,y2) {
        ...
        for (var i = 0; dist >= 0.1; i++) {
            ...
            var dl   = dist < pattern[idx] ? dist : pattern[idx],
...
export const $extendContext = function(ctx, methods){
...
export const $canvas = function(c, w, h, forceResize) {
    ...
    if (typeof ctx.$ratio === "undefined") {
        ...
        if ($deviceRatio != ctx.$ratio) {
            var r = $deviceRatio / ctx.$ratio;
        ...
        $extendContext(ctx, $ContextMethods);
    }
    ...
    if (ctx.$ratio != $deviceRatio) {
        var ratio = $deviceRatio / ctx.$ratio;
